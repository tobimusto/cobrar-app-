      ...
      <div className="flex-1 bg-cobrar-bg3 border border-cobrar-border rounded-2xl flex flex-col overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            <UsersIcon size={48} className="text-cobrar-txt3 mb-4" />
            <h3 className="text-lg font-bold text-white mb-2">No hay clientes</h3>
            <p className="text-sm text-cobrar-txt2 mb-6">Aún no has registrado ningún cliente en tu negocio.</p>
            <button 
              onClick={() => setShowModal(true)}
              className="bg-[#5252ff]/10 text-[#5252ff] hover:bg-[#5252ff]/20 font-bold py-2 px-6 rounded-lg transition-all text-sm flex items-center gap-2 border border-[#5252ff]/30"
            >
              <UserPlus size={16} />
            ...
            <table className="w-full min-w-[640px] text-left text-sm text-cobrar-txt2">
              ...
              <tbody>
                {filteredClients.map(client => (
                  <tr key={client.id} className="border-b border-cobrar-border/50 hover:bg-cobrar-bg2 transition-colors">
                    <td className="px-4 py-4 font-bold text-white">{client.nombre}</td>
                    <td className="px-4 py-4">{client.telefono}</td>
                    <td className="px-4 py-4">{client.iva}</td>
                    <td className="px-4 py-4 font-bold">
                      <div className="flex items-center gap-3">
                        <span className={client.deuda > 0 ? "text-red-400" : "text-cobrar-green"}>
                        ...
                        <button 
                          ...
                          className="p-1.5 bg-cobrar-bg border border-cobrar-border text-cobrar-txt2 hover:text-white rounded-lg transition-colors group"
                          title="Modificar deuda"
                        >
                          <Edit2 size={14} className="group-hover:text-[#5252ff]" />
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0f0f13] border border-cobrar-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <div className="p-6 border-b border-cobrar-border">
              <h2 className="text-lg font-head font-bold text-white">Nuevo Cliente</h2>
            </div>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-[#1a1a23] border border-[#5252ff]/30 rounded-lg py-2.5 px-4 text-sm focus:outline-none focus:border-[#5252ff] text-white transition-colors shadow-[0_0_10px_rgba(82,82,255,0.1)]"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-white mb-2">CUIT o DNI (Opcional)</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-white mb-2">Condición IVA *</label>
                    <div className="relative">
                      <select 
                        value={newClient.iva}
                      ...
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-cobrar-txt2">
                        ▼
                      </div>
                    </div>
                  </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-white mb-2">Provincia (Opcional)</label>
                    <div className="relative">
                      <select 
                        value={newClient.provincia}
                        onChange={e => setNewClient({...newClient, provincia: e.target.value})}
                        className="w-full bg-[#1a1a23] border border-cobrar-border rounded-lg py-2.5 px-4 text-sm focus:outline-none focus:border-[#5252ff]/50 text-cobrar-txt2 transition-colors appearance-none"
                      >
            ...
            <div className="p-4 border-t border-cobrar-border bg-cobrar-bg2 flex justify-end gap-3">
              ...
              <button 
                ...
                className="bg-[#5252ff] hover:bg-[#6666ff] font-bold text-white py-2 px-5 rounded-lg transition-all text-sm shadow-[0_4px_15px_rgba(82,82,255,0.3)] flex items-center justify-center gap-2"
              >
                Crear Cliente
              </button>
            </div>
          ...
          <div className="bg-[#0f0f13] border border-cobrar-border rounded-2xl w-full max-w-sm max-h-[90vh] overflow-y-auto shadow-2xl relative">
            <div className="p-6">
              ...
              <p className="text-sm text-cobrar-txt2 mb-6">
                Cliente: <strong className="text-white">{selectedClient.nombre}</strong><br/>
                Deuda actual: <strong className={selectedClient.deuda > 0 ? "text-red-400" : "text-cobrar-green"}>${Number(selectedClient.deuda).toLocaleString('es-AR', {minimumFractionDigits:2})}</strong>
              </p>

              <div className="flex bg-[#1a1a23] rounded-lg p-1 mb-4 border border-cobrar-border">
                <button 
                  onClick={() => setDebtAction('sumar')}
                  className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${debtAction === 'sumar' ? 'bg-[#5252ff] text-white' : 'text-cobrar-txt2 hover:text-white'}`}
                >
                  Sumar Deuda
                </button>
                <button 
                  onClick={() => setDebtAction('restar')}
                  className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${debtAction === 'restar' ? 'bg-cobrar-bg2 text-white shadow-sm' : 'text-cobrar-txt2 hover:text-white'}`}
                >
                  Abonar (Restar)
                </button>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-white mb-2">Monto a {debtAction}</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="0.00"
                    className="w-full bg-[#1a1a23] border border-cobrar-border rounded-xl py-3 pl-8 pr-4 text-white text-lg font-bold focus:outline-none focus:border-[#5252ff] transition-colors"
                    autoFocus
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button 
                  ...
                  className="flex-1 bg-[#5252ff] hover:bg-[#6666ff] text-white font-bold py-3 rounded-xl transition-all text-sm shadow-[0_4px_15px_rgba(82,82,255,0.25)]"
                >
                  Actualizar
                </button>
              </div>
